...
public class SearchServiceImpl implements SearchService
{
    ...
    @Override
    public List<Policy> findPolicyByElasticsearch(Map<String,Object> requestMap, ExtLimit limit) throws Exception {
        ...
        SearchQuery searchQuery = new NativeSearchQueryBuilder().withQuery(boolQueryBuilder)
                .withHighlightBuilder(new HighlightBuilder().preTags(HIGH_LIGHT_PRE_TAGS).postTags(HIGH_LIGHT_POST_TAGS))
                .withHighlightFields(new HighlightBuilder.Field(Policy.TITLE), new HighlightBuilder.Field(Policy.CONTENT))
                .withPageable(PageRequest.of(limit.getPageindex(), limit.getPagesize())).build();
        AggregatedPage<Policy> policies = elasticsearchTemplate.queryForPage(searchQuery, Policy.class, new SearchResultMapper() {
            ...
            public <T> AggregatedPage<T> mapResults(SearchResponse searchResponse, Class<T> aClass, Pageable pageable) {
                ...
                for (SearchHit searchHit : searchResponse.getHits()) {
                    ...
                    String updateTimeString = (String) searchHit.getSourceAsMap().get(Policy.UPDATE_TIME);
                    try{
        ...
        //然而updateTime在elasticsearch中包含了timezone信息，无法直接被@JsonFormat(pattern="yyyy-MM-dd HH:mm:ss",timezone="GMT+8")处理，这里需要注意
